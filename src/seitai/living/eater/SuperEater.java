...
public class SuperEater extends Living {
	...
	protected SuperEater(int x, int y, int hpmax, int attack, int guard,
			int speed, int size, int spine) {
		super(x, y, hpmax, attack, guard, speed, size, spine, 0);
		ai.putAI(5, new AIRandomMove(this));
		ai.putAI(7, new AIChaseTarget(this, FleshEater.class));
	}
	...
	public Living bear(Living l) {
		Living liv =  new SuperEater(pos.getX(), pos.getY(), 0, 0,0,0,0,0);
	...
	public static Living getCommonInstance(int x, int y){
		return new SuperEater(x, y, 10000, 50, 10, 10, 20, 0);
	}
